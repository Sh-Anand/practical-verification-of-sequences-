...
scanHelper : {a b : Set} → (a → b → a) → Σ (List a) (λ xs → NonEmpty xs) → b → Σ (List a) (λ xs → NonEmpty xs)
scanHelper f (a@(x ∷ xs) , nonEmpty) b = a' , (NonEmpty.itsNonEmpty)
    where
        l'  = f (last a {{ nonEmpty }}) b
        a'  = (x ∷ xs) ++ (l' ∷ [])

scanl : {a b : Set} -> (a -> b -> a) -> a -> Seq b -> Seq a
scanl f z xs = fromList (fst (foldl (scanHelper f) ((z ∷ []) , NonEmpty.itsNonEmpty) (toList xs))) 