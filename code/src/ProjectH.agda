...
import Prelude hiding (scanl, scanr, scanl1, scanr1, splitAt, take, drop, filter, lookup, zipWith, zip, zipWith3, zip3, reverse, length)
...
{-# COMPILE AGDA2HS isNotEmptyFingerTree #-}
...
lookupTree i (Deep _ pr (Single x) sf) = let spr = size pr
                                             spm = spr + size (Single x)
                                            in  if i < spr then lookupDigit i pr
                                                                else (if i < spm then (case (lookupTree (i - spr) (Single x) {IsTrue.itsTrue}) of
                                                                λ {
                                                                    (i' , xs) -> lookupNode i' xs
                                                                }
                                                            )
                                                                                 else lookupDigit (i - spm) sf)
lookupTree i (Deep _ pr (Deep vm prm mm sfm) sf) =  let spr = size pr
                                                        spm = spr + vm
                                                    in  if i < spr then lookupDigit i pr
                                                                else (if i < spm then (case (lookupTree (i - spr) (Deep vm prm mm sfm) {IsTrue.itsTrue}) of
                                                                λ {
                                                                    (i' , xs) -> lookupNode i' xs
                                                                }
                                                            )
...
splitTreeN i (Deep s pr m@EmptyT sf) = if i < spr then splitPrefixN i s pr m sf
                                    ...
                                    where
                                        spr = size pr
                                        im = i - spr
splitTreeN i (Deep s pr m@(Single x) sf) = let spr = size pr
                                               spm = spr + size m
                                               im = i - spr
...
splitTreeN i (Deep s pr m@(Deep vm prm mm sfm) sf) = let spr = size pr
                                                         spm = spr + size m
                                                         im = i - spr
                                                     in if i < spr then splitPrefixN i s pr m sf
                                                              else (if i < spm then (case (splitTreeN im m {IsTrue.itsTrue}) of 
...
splitTreeE i t@(Single x) = if i <= 0 then (EmptyT , t)
                                      else (t , EmptyT)
...
splitTreeE i (Deep s pr m@(Single x) sf) = let spr = size pr 
                                               spm = spr + size m
                                               im = i - spr
...
splitTreeE i (Deep s pr m@(Deep vv prm mm sfm) sf) = let spr = size pr
                                                         spm = spr + size m
                                                         im = i - spr
                                                in if i < spr then splitPrefixE i s pr m sf
                                                          else (if i < spm then (case splitTreeN im m {IsTrue.itsTrue} of 
...
{-# TERMINATING #-}
quot : Int -> Int -> Int
quot n m = if n > m then 1 + quot (n - m) m else 0

{-# TERMINATING #-}
rem :  Int -> Int -> Int
rem n m = if n > m then rem (n - m) m else m - n

{-# TERMINATING #-}
applicativeTree : {a : Set} -> {f : Set -> Set} -> {{Applicative f}} -> Int -> Int -> f a -> f (FingerTree a)
applicativeTree n mSize m = if n == 0 then pure EmptyT
                                      ...
                                      else (if r == 0 then deepA three (applicativeTree (q - 2) mSize' n3) three
                                                        else (if r == 1 then deepA two (applicativeTree (q - 1) mSize' n3) two
                                                                          else deepA three (applicativeTree (q - 1) mSize' n3) two))))))))
  where
    mSize' = 3 * mSize
...
replicateA : {a : Set} -> {{Applicative f}} -> Int -> f a -> f (Seq a)
...
{-# COMPILE AGDA2HS UnzipWith class #-}

instance
    NodeUnzipWith : UnzipWith Node
    NodeUnzipWith .unzipWith' f (Node2 s x y) = let fx = f x
                                                    fy = f y
                                                in Node2 s (fst fx) (fst fy) , Node2 s (snd fx) (snd fy)
    NodeUnzipWith .unzipWith' f (Node3 s x y z) = let fx = f x
                                                      fy = f y
                                                      fz = f z
                                                  in Node3 s (fst fx) (fst fy) (fst fz) , Node3 s (snd fx) (snd fy) (snd fz)
{-# COMPILE AGDA2HS NodeUnzipWith #-}

instance
    DigitUnzipWith : UnzipWith Digit
    DigitUnzipWith .unzipWith' f (One x) = let fx = f x
                                          in One (fst fx) , One (snd fx)
    DigitUnzipWith .unzipWith' f (Two x y) = let fx = f x
                                                 fy = f y
                                              in Two (fst fx) (fst fy) , Two (snd fx) (snd fy)
    DigitUnzipWith .unzipWith' f (Three x y z) = let fx = f x
                                                     fy = f y
                                                     fz = f z
                                                  in Three (fst fx) (fst fy) (fst fz) , Three (snd fx) (snd fy) (snd fz)

    DigitUnzipWith .unzipWith' f (Four x y z w) = let fx = f x
                                                      fy = f y
                                                      fz = f z
                                                      fw = f w
                                                  in Four (fst fx) (fst fy) (fst fz) (fst fw) , Four (snd fx) (snd fy) (snd fz) (snd fw)
...
instance
    ...
    FingerTreeUnzipWith .unzipWith' f (Single x) = let fx = f x 
                                                   in Single (fst fx) , Single (snd fx)
    FingerTreeUnzipWith .unzipWith' f (Deep v pr m sf) = let prf = unzipWith' f pr
                                                             sff = unzipWith' f sf
                                                             mf = unzipWith' (unzipWith' f) m
                                                         in (Deep v (fst prf) (fst mf) (fst sff)) , (Deep v (snd prf) (snd mf) (snd sff))
...
instance
    ...
    ElemUnzipWith .unzipWith' f (Element x) = let fx = f x
                                              in Element (fst fx) , Element (snd fx)
{-# COMPILE AGDA2HS ElemUnzipWith #-}

unzipWithNodeElem : {x a b : Set} -> (x -> a × b) -> Node (Elem x) -> Node (Elem a) × Node (Elem b)
unzipWithNodeElem f (Node2 s (Element x) (Element y)) = let fx = f x
                                                            fy = f y
                                                        in Node2 s (Element (fst fx)) (Element (fst fy)) , Node2 s (Element (snd fx)) (Element (snd fy))

unzipWithNodeElem f (Node3 s (Element x) (Element y) (Element z)) = let fx = f x
                                                                        fy = f y
                                                                        fz = f z
                                                                    in Node3 s (Element (fst fx)) (Element (fst fy)) (Element (fst fz)) , Node3 s (Element (snd fx)) (Element (snd fy)) (Element (snd fz))
{-# COMPILE AGDA2HS unzipWithNodeElem #-}

instance
    SeqUnzipWith : UnzipWith Seq
    SeqUnzipWith .unzipWith' f (Sequence EmptyT) = (Sequence EmptyT) , (Sequence EmptyT)
    SeqUnzipWith .unzipWith' f (Sequence (Single (Element x))) = let fx = f x
                                                                 in (Sequence (Single (Element (fst fx)))) , (Sequence (Single (Element (snd fx))))
    SeqUnzipWith .unzipWith' f (Sequence (Deep v pr m sf)) = let prf = unzipWith' (unzipWith' f) pr
                                                                 sff = unzipWith' (unzipWith' f) sf
                                                                 mf = unzipWith' (unzipWithNodeElem f) m
                                                             in Sequence (Deep v (fst prf) (fst mf) (fst sff)) , Sequence (Deep v (snd prf) (snd mf) (snd sff))
